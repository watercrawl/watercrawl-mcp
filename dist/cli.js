...
// src/server/index.ts
var import_fastmcp7 = require("fastmcp");
...
// src/utils/client.ts
var import_nodejs = require("@watercrawl/nodejs");
var getClient = (apiKey = "") => {
  const BASE_URL = process.env.WATERCRAWL_BASE_URL || "https://app.watercrawl.dev";
  const API_KEY = apiKey || process.env.WATERCRAWL_API_KEY || "";
  return new import_nodejs.WaterCrawlAPIClient(API_KEY, BASE_URL);
};

// src/tools/scrape.ts
var scrapeUrl = async (args, { session }) => {
  const client = getClient(session == null ? void 0 : session.apiKey);
  try {
    ...
    throw new import_fastmcp.UserError(String(e));
  }
};
...
// src/tools/search.ts
var import_fastmcp2 = require("fastmcp");
var search = async (args, { session }) => {
  const client = getClient(session == null ? void 0 : session.apiKey);
  try {
    ...
    throw new import_fastmcp2.UserError(String(e));
  }
};
var parameters2 = z.object({
...
var import_fastmcp3 = require("fastmcp");
var downloadSitemap = async (args, { session }) => {
  const client = getClient(session == null ? void 0 : session.apiKey);
  try {
    switch (args.format) {
      case "graph":
    ...
    throw new import_fastmcp3.UserError(String(e));
  }
};
var parameters3 = z.object({
...
var import_fastmcp4 = require("fastmcp");
var manageCrawl = async (args, { session }) => {
  const client = getClient(session == null ? void 0 : session.apiKey);
  try {
    switch (args.action) {
      case "list":
      ...
      case "get":
        if (!args.crawlRequestId) {
          throw new import_fastmcp4.UserError("crawlRequestId is required for 'get' action");
        }
        const getResult = await client.getCrawlRequest(args.crawlRequestId);
        return JSON.stringify(getResult);
      case "stop":
        if (!args.crawlRequestId) {
          throw new import_fastmcp4.UserError("crawlRequestId is required for 'stop' action");
        }
      ...
      case "download":
        if (!args.crawlRequestId) {
          throw new import_fastmcp4.UserError("crawlRequestId is required for 'download' action");
        }
        const downloadResult = await client.downloadCrawlRequest(args.crawlRequestId);
        return JSON.stringify(downloadResult);
      default:
        throw new import_fastmcp4.UserError(`Unknown action: ${args.action}`);
    }
  } catch (e) {
    throw new import_fastmcp4.UserError(String(e));
  }
};
var parameters4 = z.object({
...
var import_fastmcp5 = require("fastmcp");
var manageSearch = async (args, { session }) => {
  const client = getClient(session == null ? void 0 : session.apiKey);
  try {
    switch (args.action) {
      case "list":
      ...
      case "stop":
        if (!args.searchRequestId) {
          throw new import_fastmcp5.UserError("searchRequestId is required for 'stop' action");
        }
      ...
      default:
        throw new import_fastmcp5.UserError(`Unknown action: ${args.action}`);
    }
  } catch (e) {
    throw new import_fastmcp5.UserError(String(e));
  }
};
var parameters5 = z.object({
...
var import_fastmcp6 = require("fastmcp");
var monitor = async (args, { session }) => {
  const client = getClient(session == null ? void 0 : session.apiKey);
  ...
  try {
    ...
    if (args.type === "crawl") {
      ...
      throw new import_fastmcp6.UserError(`Unknown monitor type: ${args.type}`);
    }
    return JSON.stringify({
      status: "completed",
      events
    ...
    throw new import_fastmcp6.UserError(String(e));
  }
};
var parameters6 = z.object({
...
var authenticate = async (request) => {
  ...
  if (request.headers.authorization) {
    ...
    const parsed_query = new URLSearchParams((_a = request.url) == null ? void 0 : _a.split("?")[1]);
...
// src/server/index.ts
var setupServer = () => {
  const server = new import_fastmcp7.FastMCP({
...
program2.name("watercrawl-mcp").description("WaterCrawl MCP (Model Context Protocol) Server").version("1.0.0");
program2.option("-b, --base-url <url>", "WaterCrawl API base URL", "https://app.watercrawl.dev").option("-k, --api-key <key>", "WaterCrawl API key");
program2.command("sse").description("Start the server with SSE transport").option("-p, --port <number>", "Port to run the SSE server on", "3000").option("-e, --endpoint <path>", "SSE endpoint path", "/sse").action((options) => {
  if (program2.opts().baseUrl) {
...
program2.command("stdio", { isDefault: true }).description("Start the server with STDIO transport (default)").action(() => {
  if (program2.opts().baseUrl) {
    process.env.WATERCRAWL_BASE_URL = program2.opts().baseUrl;
  }
  if (program2.opts().apiKey) {
